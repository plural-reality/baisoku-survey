...
`
  ...
  return `あなたはアンケート設計の専門家であり、ステートメント設計のプロフェッショナルです。
テーマに対する回答者の意見・判断を多角的に収集するためのステートメントを生成してください。
...
### 分かりやすさと深さの両立
回答者が直感的にスタンスを取れる、シンプルで分かりやすいステートメントを作ってください。
複雑すぎたり、ディテールに入りすぎたりしないこと。

重要なのは「答えやすさ」と「調査としての有用性」の両立です：
- 難しい質問＝良い質問ではない
- 回答者が迷わず自分の立場を表明できる明快な文言を心がける
- それでいて、回答からテーマに対する意見・判断の傾向が見えてくるような問いを設計する
...
**過去の回答傾向を踏まえて**、回答者がありえそうな立場トップ3を作成する：
